...
class WordService {
  Future<bool> deleteWord(int userID, int wordID) async {
    ...
    try {
      final response = await http.delete(
        ...
        body: jsonEncode({'userID': userID, 'wordID': wordID}),
      );
      if (response.statusCode == 200) {
        return true;
      } else {
        debugPrint("❌ Lỗi API xóa từ: ${response.body}");
        return false;
      }
    } catch (e) {
      debugPrint("❌ Lỗi mạng khi xóa từ: $e");
      return false;
    }
  }
  ...
  Future<bool> updateWord(
    ...
    final url = Uri.parse('$baseUrl/api/word/updateWord');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("❌ Lỗi API cập nhật từ: ${response.body}");
        return false;
      }
    } catch (e) {
      debugPrint("❌ Lỗi mạng khi cập nhật từ: $e");
      return false;
    }
  }
  ...
  Future<bool> addWordToLibrary(Word word, int userID) async {
    final url = Uri.parse('$baseUrl/api/word/addWord');
  ...
  Future<List<Word>> getUserWords(int userId) async {
    ...
    final url = Uri.parse('$baseUrl/api/word/$userId');
