...
class _ExerciseContainerPageState extends State<ExerciseContainerPage> {
  ...
  Future<void> _saveAndExit() async {
    ...
    // 1. Lưu lịch sử làm bài và cập nhật SM-2 trước
    bool success = await ExerciseService.saveExerciseResult(
      widget.exerciseSession,
      userID,
    );

    // 2. Nếu lưu bài tập thành công, tiến hành cộng EXP và Streak
    if (success) {
      // Công thức: 10 EXP cho mỗi câu đúng
      int earnedExp = widget.exerciseSession.nCorrectAnswer * 10;
      final int userId = currentStudentNotifier.value!.userID;

      // Sử dụng Future.wait để chạy TẤT CẢ các API cùng một lúc

      // Sử dụng Future.wait để gọi các API cùng một lúc giúp load nhanh hơn
      await Future.wait([
        // Chỉ cộng EXP nếu có ít nhất 1 câu đúng
        if (earnedExp > 0) StudentService.updateTotalExp(userID, earnedExp),
        if (earnedExp > 0) StudentService.updateWeeklyExp(userID, earnedExp),

        // Luôn gọi updateStreak để "điểm danh" ngày học (dù làm đúng hay sai)
    ...
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "Đã lưu kết quả! Bạn nhận được ${widget.exerciseSession.nCorrectAnswer * 10} EXP",
          ),
